...
class GameWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # ========== Money display (with profit prompt)==============
    ...
    def show_money_change(self, amount, source=""):
        ...
        if amount == 0:
            # 显示 "study" 提示
            self.money_change_label.setText("(study)")
            self.money_change_label.setStyleSheet("""
                QLabel {
                    color: #7f8c8d;
                    font-size: 11px;
            ...
            """)
            QTimer.singleShot(500, lambda: self.money_change_label.setText(""))
            return
    ...
    def called(self, reviewer, card, ease):
        ...
        if bonus > 0:
            ...
            # 随机奖励 - 如果有产物，合并显示；如果没有，立即显示
            if total_production > 0:
                # 合并到现有提示
                current_text = self.money_change_label.text()
                self.money_change_label.setText(f"{current_text}, +{bonus}(bonus)")
            else:
                self.update_money_display(bonus, "bonus")
